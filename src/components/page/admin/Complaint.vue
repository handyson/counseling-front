                ...
                <el-table-column label="操作" align="center" width="250">
                    <template slot-scope="scope">
                        ...
                        <el-button
                                type="danger"
                                icon="el-icon-lx-roundclose"
                                @click="bohuitousu(scope.$index, scope.row)"
                        >驳回投诉
                        </el-button>
                    </template>
                </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            fenghao(index, row) {
                axios.get('/api/complaint/blockUser?id=' + row.complaint_id).then(res => {
                ...
                axios.get('/api/complaint/reject?id=' + row.complaint_id).then(res => {
                    if (res.data.code == 200)
                        this.$message.success(res.data.msg)
                    else this.$message.error(res.data.msg)
                    this.getData()
                ...
                for (let i = 0; i < length; i++) {
                    axios.get('/api/complaint/deleteByFlag?id=' + this.multipleSelection[i].complaint_id).then(res => {
                    }).catch(error => {
                        console.log("接口请求异常");
                    });
                }
                ...
                axios.post('/api/complaint/selectKeyByLimit', this.query).then(res => {
