...
<script>
    ...
    export default {
        ...
        methods: {
            deleterow(index, row) {
                axios.get('/api/logsinfo/deleteByFlag?id=' + row.logid).then(res => {
                ...
                for (let i = 0; i < length; i++) {
                    axios.get('/api/logsinfo/deleteByFlag?id=' + this.multipleSelection[i].logid).then(res => {
                    }).catch(error => {
                        console.log("接口请求异常");
                    });
                }
                ...
                axios.post('/api/logsinfo/selectKeyByLimit', this.query).then(res => {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    this.tableData[i].createtimeString = createtime.getFullYear() + "年" + month + "月" + createtime.getDate() + "日 " + createtime.getHours() + "时" + createtime.getMinutes()+"分";
                }
            },
            // 分页导航
