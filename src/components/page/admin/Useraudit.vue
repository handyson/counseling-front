    ...
    <div>
        <div class="crumbs">
            <el-breadcrumb separator="/">
                <el-breadcrumb-item>
                    <i class="el-icon-lx-cascades"></i> 咨询师审核
                </el-breadcrumb-item>
            </el-breadcrumb>
        </div>
        <div class="container">
                ...
                <el-table-column label="操作" align="center">
                    ...
                    >
                        <el-button
                                ...
                                type="success"
                                @click="PassAudit(scope.$index, scope.row)"
                        >申请通过
                        </el-button>
                        <el-button
                                type="danger"
                                icon="el-icon-lx-roundclose"
                                @click="ReMoveAudit(scope.$index, scope.row)"
                        >驳回申请
                        </el-button>
                    </template>
                </el-table-column>
    ...
    export default {
        ...
        methods: {
            getData() {
                ...
                axios.post('/api/audit/selectKeyByLimit', this.query).then(res => {
                    console.log(res);
            ...
            PassAudit(index, row) {
                if (row.auditstate == '1') {
                    ...
                    axios.get('/api/audit/PassAudit?id=' + row.auditid).then(res => {
                        this.getData();
                    }).catch(error => {
                        console.log("接口请求异常");
                    });
            ...
            ReMoveAudit(index, row) {
                if (row.auditstate == '0') {
                    ...
                    axios.get('/api/audit/ReMoveAudit?id=' + row.auditid).then(res => {
                        this.getData();
                    }).catch(error => {
                        console.log("接口请求异常");
                    });
            ...
            changeData() {
                ...
                for (let i = 0; i < length; i++) {
                    ...
                    this.tableData[i].logintimeString = logintime.getFullYear() + "-" + month + "-" + logintime.getDate() + " " + logintime.getHours() + ":" + logintime.getMinutes();
                }
            },
            // 分页导航
