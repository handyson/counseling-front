<template>
  <div class="audio-call-section">
    ...
    >Welcome {{currentUser && (currentUser.nickname || currentUser.id)}}</div>
    ...
    <div :class="{ 'audio-conference': true, 'is-show': isShowAudioCall }">
      ...
      <div class="audio-conference-list">
        <div
          ...
          :key="`audio-${userId}`"
          :class="{'user-audio-container': true, 'is-me': userId === currentUser.id}"
        >
...
import { getUserDetailInfoByUserid } from "../../../utils/trtc";
...
export default {
  ...
  mounted() {
    ...
    console.log("call audio to user ID", param);
    if (param) {
      param = String(param);
