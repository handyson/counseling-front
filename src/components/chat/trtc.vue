...
export default {
    ...
    methods: {
        ...
        handleNewInvitationReceived: async function (payload) {
            ...
            //         "roomID":10795622
            //     },
            //     "inviteID":"7f6197e9-ede8-4de7-af5a-dc9804229dc3"
            // }
            ...
            if (sponsor === this.currentUser.id) {
            ...
            const userName = await getUsernameByUserid(sponsor);
        ...
        handleAcceptCall: async function () {
            try {
                ...
                if (callType === this.TrtcCalling.CALL_TYPE.AUDIO_CALL && this.$router.history.current.fullPath !== '/audio-call') {
                    this.$router.push('/audio-call');
                } else if (callType === this.TrtcCalling.CALL_TYPE.VIDEO_CALL && this.$router.history.current.fullPath !== '/video-call') {
        ...
        handleUserVideoChange: function ({ userID, isVideoAvailable }) {
            log(`handleUserVideoChange userID, ${userID} isVideoAvailable ${isVideoAvailable}`);
            if (isVideoAvailable) {
                const muteUserList = this.muteAudioUserIdList.filter((currentID) => currentID !== userID);
        ...
        handleUserAudioChange: function ({ userID, isAudioAvailable }) {
            log(`handleUserAudioChange userID, ${userID} isAudioAvailable ${isAudioAvailable}`);
            if (isAudioAvailable) {
                const muteUserList = this.muteAudioUserIdList.filter((currentID) => currentID !== userID);
