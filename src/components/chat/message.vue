		...
		<ul>
			<li v-for="entry in sessions['群聊']" :key="entry.id">
				...
				<div class="main" :class="{self:entry.fromId==user.id}">
					...
					<img @dblclick="takeAShot" class="avatar" :src="entry.fromId==user.id? user.avatar:entry.fromAvatar" alt="">
					<template v-if="(entry.messageTypeId==1)"><p class="text" v-html="entry.content"></p></template>
					<template v-else>
            <!--图片预览与无法加载图片的图标-->
